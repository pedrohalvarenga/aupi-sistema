...
function CadastroPublicoForm() {
  ...
  const empresaSlug = searchParams.get('e') ?? ''
  const [empresa, setEmpresa] = useState<EmpresaPublica | null>(null)
  const [empresaErro, setEmpresaErro] = useState(false)

  useEffect(() => {
    if (!empresaSlug) { setEmpresaErro(true); return }
    fetch(`/api/empresa-publica?slug=${encodeURIComponent(empresaSlug)}`)
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then(setEmpresa)
      .catch(() => setEmpresaErro(true))
  }, [empresaSlug])
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-orange-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-xl">
          ...
          <p className="text-gray-500">
            <strong>{listaNomes}</strong> {nomesPets.length > 1 ? 'estão cadastrados' : 'está cadastrado(a)'} na {empresa?.nome ?? 'nossa equipe'}. Em breve entraremos em contato.
          </p>
          <div className="mt-6 p-4 bg-purple-50 rounded-2xl">
            <p className="text-xs text-purple-600 font-medium">Guarde nosso número</p>
            <p className="text-sm text-gray-700 mt-1">{empresa?.whatsapp ? `WhatsApp: ${empresa.whatsapp}` : `WhatsApp ${empresa?.nome ?? ''}`}</p>
          </div>
        </div>
      </div>
    )
  ...
  if (empresaErro) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-gray-50">
        <div className="bg-white rounded-3xl p-8 max-w-sm text-center shadow-sm border border-gray-100">
          <h1 className="text-xl font-bold text-gray-900 mb-2">Link inválido</h1>
          <p className="text-sm text-gray-500">
            Este link de cadastro não está ativo. Peça o link correto para a empresa onde seu pet ficará.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-orange-50" style={tema}>
      {/* Header */}
      <div className="bg-brand-purple text-white px-4 py-6 text-center">
        <div className="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-3">
          <Dog size={28} />
        </div>
        <h1 className="text-2xl font-bold">{empresa?.nome ?? 'Cadastro'}</h1>
