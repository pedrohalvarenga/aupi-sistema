...
import { Send, Sparkles, ArrowLeft, UploadCloud } from 'lucide-react'
...
const SAUDACAO: Msg = {
  role: 'assistant',
  content: 'Oi! Sou a assistente do Aupipet 🐾 Posso te ajudar com qualquer coisa do sistema na hora — cadastros, agenda, cobrança, relatórios, importação de dados e mais. O que você precisa?',
}
...
export default function SuportePage() {
  ...
  useEffect(() => { fimRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [messages])
  ...
  return (
    <div className="py-6 flex flex-col gap-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Assistente Aupipet</h1>
          <p className="text-sm text-gray-400">Tire qualquer dúvida do sistema na hora, 24/7</p>
        </div>
      </div>

      {/* Janela do chat */}
      <div className="bg-white rounded-3xl border border-gray-100 p-4 flex flex-col gap-3 min-h-[55vh]">
        {messages.map((m, i) => (
      ...
      {/* Atalho para importação — pedido frequente, 100% self-service */}
      <Link href="/importar" className="flex items-center justify-center gap-2 py-3 rounded-2xl font-semibold text-sm text-brand-orange border border-brand-orange/40 bg-orange-50/50">
        <UploadCloud size={17} /> Importar meus dados (planilha, PDF ou foto)
      </Link>
    </div>
  )
}
