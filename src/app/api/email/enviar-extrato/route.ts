...
export async function POST(request: Request) {
  ...
  // Chamada pelo cron (com segredo) usa service role; chamada pelo app usa a sessão (RLS escopa a empresa)
  ...
  // Marca da empresa (white label) no e-mail
  const { data: empresa } = await supabase
    .from('empresas')
    .select('nome, cidade, cor_primaria')
    .eq('id', tutor.empresa_id)
    .single()
  const emp = empresa ?? { nome: 'Sua creche', cidade: '', cor_primaria: '#8A05BE' }
...
function gerarHtmlExtrato({
  tutor,
  empresa, pets, presencas, compras, mes, ano }: {
  tutor: Record<string, string>
  empresa: { nome: string; cidade?: string | null; cor_primaria: string }
...
<div style="max-width:600px;margin:32px auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,.08);">
  <!-- Header -->
  <div style="background:${empresa.cor_primaria};padding:32px;text-align:center;">
    <h1 style="margin:0;color:white;font-size:28px;font-weight:800;letter-spacing:-0.5px;">🐾 ${empresa.nome}</h1>
    <p style="margin:8px 0 0;color:rgba(255,255,255,.8);font-size:14px;">${empresa.cidade ?? ''}</p>
  ...
  <div style="padding:24px 32px;">
    <div style="display:flex;gap:12px;margin-bottom:24px;">
      <div style="flex:1;background:#f3f4f6;border-radius:12px;padding:16px;text-align:center;">
        <p style="margin:0;font-size:28px;font-weight:800;color:${empresa.cor_primaria};">${totalPresencas}</p>
  ...
  <div style="padding:20px 32px;background:#f9fafb;border-top:1px solid #f3f4f6;text-align:center;">
    <p style="margin:0;font-size:12px;color:#9ca3af;">${empresa.nome}${empresa.cidade ? ' · ' + empresa.cidade : ''} · Este é um e-mail automático</p>
  </div>
</div>
</body></html>`
}
