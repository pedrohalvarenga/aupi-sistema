...
export async function POST(request: Request) {
  ...
  // Resolve a empresa pelo slug do link de cadastro (?e=slug)
  if (!empresaSlug) {
  ...
  const { data: empresa } = await adminClient
    ...
    .eq('slug', empresaSlug)
    .maybeSingle()
  if (!empresa || empresa.status === 'cancelado' || empresa.status === 'suspenso') {
    return NextResponse.json({ error: 'Cadastro indisponível para esta empresa' }, { status: 400 })
  }
