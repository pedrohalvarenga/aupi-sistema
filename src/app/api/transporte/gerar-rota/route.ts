...
// endereço de partida da empresa. Sem chave configurada, cria a rota na ordem
// da lista e avisa.
...
// Distância da sede até cada parada (base para precificação futura)
...
export async function POST(req: Request) {
  ...
  const partida = config?.valor
  if (!partida) {
    return NextResponse.json({ error: 'Configure o endereço de partida em Transportes → Veículo.' }, { status: 400 })
  }
